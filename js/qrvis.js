...
(function (factory) {
    ...
    function (vg, d3, topojson) {
        ...
        qrvis.number = function (s) {
            return +s;
        };

        qrvis.boolean = function (s) {
            return !!s;
        };

        // utility functions

        qrvis.identity = function (x) {
            return x;
        };
        ...
        qrvis.accessor = function (f) {
            var s;
            return (qrvis.isFunction(f) || f == null) ? f : qrvis.isString(f) && (s = qrvis.field(f)).length > 1 ? function (x) {
        ...
        qrvis.mutator = function (f) {
            var s;
            return qrvis.isString(f) && (s = qrvis.field(f)).length > 1 ? function (x, v) {
        ...
        qrvis.cmp = function (a, b) {
            return a < b ? -1 : a > b ? 1 : 0;
        };

        qrvis.numcmp = function (a, b) {
            return a - b;
        };
        ...
        qrvis.str = function (x) {
            return qrvis.isArray(x) ? "[" + x.map(qrvis.str) + "]" : qrvis.isObject(x) ? JSON.stringify(x) : qrvis.isString(x) ? ("'" + qrvis_escape_str(x) + "'") : x;
        };
        ...
        qrvis.truncate = function (s, length, pos, word, ellipsis) {
            ...
            case "center":
                var l1 = Math.ceil(l / 2),
                    l2 = Math.floor(l / 2);
                return (word ? qrvis_truncateOnWord(s, l1) : s.slice(0, l1)) + ellipsis + (word ? qrvis_truncateOnWord(s, l2, 1) : s.slice(len - l2));
            default:
        ...
        function qrvis_truncateOnWord(s, len, rev) {
            var cnt = 0,
                tok = s.split(qrvis_truncate_word_re);
            if (rev) {
                s = (tok = tok.reverse())
                    .filter(function (w) {
                        cnt += w.length;
        ...
        function qrvis_write(msg) {
            qrvis.config.isNode ? process.stderr.write(msg + "\n") : console.log(msg);
        }
        ...
        qrvis.generate = (function () {
            ...
            var generator = function (id, parentID, n, data, w, h, QR_DELAY) {
                ...
                var element = document.getElementById("animCanvas");
